...
function draw() {
  ...
  for (var i = 0; i < playerArrows.length;i++) {
    ...
    for (var j = 0; j < computerArcher.length; j++) {
      if (playerArrows[i] !== undefined && computerArcher[j] !== undefined) {
        var collision = Matter.SAT.collides(playerArrows[i].body, computerArcher[j].body);
        if (collision.collided) {
          computerArcherLife = -1[j].remove(j);             
